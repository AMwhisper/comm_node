...
PROTOCOL_HEADER = 0xA5  # 帧头
CMD_ID_AUTOAIM = 0X0401  # 自瞄命令ID

def pack_autoaim_data(yaw_angle_diff, pitch_angle_diff, fire, seq):
    # 1. 将数据打包成 autoaim_data_t 格式
    autoaim_data = struct.pack('<ffB', yaw_angle_diff, pitch_angle_diff, fire)
    data_length = len(autoaim_data)  # 9 字节

    # 2. Header
    packet = bytearray()
    packet.append(PROTOCOL_HEADER)  # 帧头 SOF
    packet.extend(struct.pack('<H', data_length))  # 数据长度
    ...
    # 4. Cmd ID
    packet.extend(struct.pack('<H', CMD_ID_AUTOAIM))

    # 5. 加入 autoaim 数据
    packet.extend(autoaim_data)

    # 6. CRC16 校验整个数据包
    crc16 = get_crc16_checksum(packet)
